...
func naturalNumberSum(num int) int {
	var total int
	var i int
	for i = 1; i <= num; i++ {
		total = total + i
	...
	for _, number := range numberArray {
		if outerRing == 0 {
			if medianValue >= number {
				diff := (medianValue - number)
				highAmount += naturalNumberSum(diff)
			}
			if medianValue <= number {
				diff := (number - medianValue)
				highAmount += naturalNumberSum(diff)
			}
		} else {
			...
			if highRing >= number {
				diff := (highRing - number)
				highAmount += naturalNumberSum(diff)
			}
			if highRing < number {
				diff := (number - highRing)
				highAmount += naturalNumberSum(diff)
			}
			if lowRing > number {
				diff := (lowRing - number)
				lowerAmount += naturalNumberSum(diff)
			}
			if lowRing <= number {
				diff := (number - lowRing)
				lowerAmount += naturalNumberSum(diff)
